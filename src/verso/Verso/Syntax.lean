...
This module contains an internal syntax that's used to represent Verso documents.
...
There are a variety of ways of using Verso's parser in Lean code. They are defined in
`Verso.Doc.Concrete`, of which `#docs` and `#doc` are most likely to be generally useful.
-/
...
/-- Custom directive -/
syntax (name:=directive) ":::" rawIdent argument* "{" block:max* "}" : block
/-- A header -/
