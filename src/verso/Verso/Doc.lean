...
into a value by invoking the `VersoDoc.toPart` method.
-/
public structure VersoDoc (genre : Genre) where
  private construct : Unit → Part genre

instance : Inhabited (VersoDoc genre) where
  default := VersoDoc.mk fun () => Inhabited.default

/--
Constructs a new VersoDoc; this function wraps the private {lean}`VersoDoc.mk` constructor. Only
code generated by {lit}`Concrete.lean ` should refer to this function.
-/
public def VersoDoc.new : (Unit → Part genre) → VersoDoc genre :=
  VersoDoc.mk
