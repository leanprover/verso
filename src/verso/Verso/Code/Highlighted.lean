          ...
          {← hypotheses.mapM fun
              | ⟨names, t, _⟩ => do pure html%{
                  <span class="hypothesis">
                    <span class="name">{(← names.mapM (·.toHtml)).toList.intersperse html%{{" "}} }</span><span class="colon">:</span>
...
hovers and highlighting other occurrences of an identifier.

The parameter `highlightJsonPromise` is a JavaScript expression that evaluates to a promise
that provides the hover content.
-/
