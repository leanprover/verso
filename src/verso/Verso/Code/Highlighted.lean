...
Pages that contain highlighted code should include these rules in a stylesheet. The colors and
fonts that the rules apply are controlled by CSS variables, documented in `verso-vars.css`.
-/
...
/--
A JavaScript expression that fetches hover content from the path named by the nearest enclosing
element that carries `data-verso-docs`, falling back to `-verso-docs.json` in the site root for a
container that has none.

This is the default means by which the code generated by `highlightingJs` obtains hover content.
The expression is evaluated with the JavaScript variable `versoRoot` bound to the wrapper that the
script is initializing, so the path travels with the content that needs it and one document holds as
many sources as it holds wrappers.
-/
public def fetchDocsJson : String :=
"fetch(versoRoot.closest(\"[data-verso-docs]\")?.dataset?.versoDocs ?? \"-verso-docs.json\").then((resp) => resp.json())"

/--
JavaScript code that implements the interactive features of highlighted Lean code, displaying
hovers and highlighting other occurrences of an identifier, within each element that matches
`selector`.

The parameter `highlightJsonPromise` is a JavaScript expression that evaluates to a promise
that provides the hover content. It is evaluated with the JavaScript variable `versoRoot` bound to
the wrapper being initialized.

The script confines its queries, its listeners, and the data it fetches to those subtrees, and it
registers its listeners additively, so a page carries the scripts of several Verso releases without
any of them reaching each other's markup.
-/
public def highlightingJs
    (selector : String := "body")
    (highlightJsonPromise : String := fetchDocsJson) : String :=
"
window.addEventListener(\"load\", () => {
  for (const versoRoot of document.querySelectorAll(" ++ selector.quote ++ ")) {
    if (versoRoot.dataset.versoCodeReady) { continue; }
    versoRoot.dataset.versoCodeReady = \"true\";
    versoInitCode(versoRoot);
  }

  async function versoInitCode(versoRoot) {

    // An element belongs to the nearest enclosing element carrying `verso-content`, so a wrapper
    // nested inside another wrapper is left to its own release's script.
    function versoOwns(el) {
    ...
    function versoQuery(selector) {
      return Array.from(versoRoot.querySelectorAll(selector)).filter(versoOwns);
    }

    // Don't show hovers inside of closed tactic states
    function blockedByTactic(elem) {
      let parent = elem.parentNode;
      while (parent && versoRoot.contains(parent) && \"classList\" in parent) {
    ...
    versoRoot.addEventListener('mouseover', (e) => { hoverTarget = e.target; }, true);
    ...
    function innermostShowable(el) {
      while (el && versoRoot.contains(el) && el.nodeType === Node.ELEMENT_NODE) {
    ...
    function getBindingTokens(context, binding) {
      ...
      if (!tokens) {
        tokens = [];
        for (const example of versoQuery(\".hl.lean\")) {
    ...
    for (const container of versoQuery(\".hl.lean\")) {
    ...
    if ('undefined' !== typeof marked) {
        for (const d of versoQuery(\"code.docstring, pre.docstring\")) {
    ...
    // Add hovers
    const versoCodeBlocks = versoQuery(\".hl.lean\");
    const versoDocData = versoCodeBlocks.length === 0 ? {} : await (" ++ highlightJsonPromise ++ ");

    function hideParentTooltips(element) {
      let parent = element.parentElement;
      while (parent && versoRoot.contains(parent)) {
    ...
    versoQuery('.hl.lean .const.token, .hl.lean .keyword.token, .hl.lean .literal.token, .hl.lean .option.token, .hl.lean .var.token, .hl.lean .typed.token, .hl.lean .level-var, .hl.lean .level-const, .hl.lean .level-op, .hl.lean .sort').forEach(element => {
      element.setAttribute('data-tippy-theme', 'lean');
    });
    versoQuery('.hl.lean .has-info.warning').forEach(element => {
      element.setAttribute('data-tippy-theme', 'warning message');
    });
    versoQuery('.hl.lean .has-info.information').forEach(element => {
      element.setAttribute('data-tippy-theme', 'info message');
    });
    versoQuery('.hl.lean .has-info.error').forEach(element => {
      element.setAttribute('data-tippy-theme', 'error message');
    });
    versoQuery('.hl.lean .tactic').forEach(element => {
      element.setAttribute('data-tippy-theme', 'tactic');
    });
    // Skip tokens inside closed tactics — they interfere with tactic tippys
    const closedTactics = new Set();
    versoQuery('.hl.lean .tactic').forEach(tactic => {
    ...
    tippy(versoQuery(tokenSelector).filter(el => !isInsideClosedTactic(el)), defaultTippyProps);
