...
private def decodeLookup : Array UInt32 := Id.run do
  ...
  for h : i in [0:85] do
    ...
    let char := alphabet[i]'(by cases h; simp_all)
...
private def alphabetChar (i : UInt32) (h : i < 85 := by bv_decide) : Char :=
  alphabet[i.toNat]'(by grind [UInt32.lt_iff_toNat_lt])
...
-/
public def encode (data : ByteArray) : String := Id.run do
...
-/
public def decode (encoded : String) (originalSize : Nat) : ByteArray := Id.run do
