...
def traverse (logError : String → IO Unit) (text : Part Manual) (config : Config) : ReaderT ExtensionImpls IO (Part Manual × TraverseState) := do
  ...
  let assets := config.features.toArray.foldr (init := config.toHtmlAssets) (·.addAssets ·)
  let mut state : Manual.TraverseState := .initialize assets remoteContent
  let mut text := text
...
def emitTeX (logError : String → IO Unit) (config : Config) (text : Part Manual) : ReaderT ExtensionImpls IO Unit := do
  ...
  let ctxt := {logError}
  let tex_ctxt := default
  let frontMatter ← text.content.mapM (·.toTeX (opts, ctxt, state, tex_ctxt))
  let chapters ← text.subParts.mapM (·.toTeX (opts, ctxt, state, tex_ctxt))
