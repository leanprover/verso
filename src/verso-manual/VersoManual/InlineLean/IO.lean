...
meta import VersoManual.InlineLean.IO.Context
import VersoManual.InlineLean.IO.Data
public meta import VersoManual.InlineLean.IO.Data
...
public def Block.exampleFile (type : FileType) : Block where
  data := ToJson.toJson type

public structure ExampleFileConfig where
  type : FileType
  «show» : Bool := true

public meta def FileType.parse [Monad m] [MonadError m] : ArgParse m FileType :=
...
public meta instance : FromArgs ExampleFileConfig m where
  fromArgs := ExampleFileConfig.mk <$> FileType.parse <*> (.flag `show true)

end

@[code_block]
public meta def exampleFile : CodeBlockExpanderOf ExampleFileConfig
  | config, str => do
    let s := str.getString
    if config.show then
...
public block_extension Block.exampleLeanFile (filename : String) where
  data := .str filename
...
public meta instance : FromArgs Config m where
  fromArgs := Config.mk <$> .named `tag .string true <*> (.flag `show true)

public structure FileConfig extends Config where
  name : String

public meta instance : FromArgs FileConfig m where
  fromArgs := FileConfig.mk <$> fromArgs <*> .positional `name .string
...
open IOExample in
@[code_block]
public meta def inputFile : CodeBlockExpanderOf FileConfig
  | opts, str => do
...
open IOExample in
@[code_block]
public meta def outputFile : CodeBlockExpanderOf FileConfig
  | opts, str => do
...
open IOExample in
@[code_block]
public meta def stdin : CodeBlockExpanderOf Config
  | opts, str => do
    saveStdin str
    -- The quote step here is to prevent the editor from showing document AST internals when the
    -- cursor is on the code block
...
open IOExample in
@[code_block]
public meta def stdout : CodeBlockExpanderOf Config
  | opts, str => do
    saveStdout str
    -- The quote step here is to prevent the editor from showing document AST internals when the
    -- cursor is on the code block
...
open IOExample in
@[code_block]
public meta def stderr : CodeBlockExpanderOf Config
  | opts, str => do
    saveStderr str
    -- The quote step here is to prevent the editor from showing document AST internals when the
    -- cursor is on the code block
...
open IOExample in
@[code_block]
public meta def ioLean : CodeBlockExpanderOf Config
  | opts, str => do
...
open IOExample in
@[directive ioExample]
public meta def ioExample : DirectiveExpanderOf Unit
 | (), blocks => do
    startExample
