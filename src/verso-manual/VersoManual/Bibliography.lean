...
private def andList (xs : Array Html) : Html :=
  if h : xs.size = 0 then panic! "tried to construct empty list of authors"
    ...
    open Html in
    (xs.extract 0 (xs.size - 1)).foldr (init := html%{{.text " and "}{xs.back} }) (· ++ ", " ++ ·)
...
def Citable.bibHtml [Monad m]
  ...
  | .inProceedings p =>
    ...
    return html%{ {authors}{s!", {p.year}. "}{ link html%{“{← go p.title}”} }. In <em>{← go p.booktitle}.</em>{(← p.series.mapM go).map (html%{{.text " ("}{·}) }) |>.getD .empty} }
  | .article p =>
    let authors ← andList <$> p.authors.mapM go
    ...
    let pagesHtml : Html := p.pages.map (fun (x, y) => .text s!" , pp. {x}–{y}")
                             |>.getD .empty
    return html%{ {authors}{.text " ("}{(← p.month.mapM go).map (· ++ .text " ") |>.getD .empty}{toString p.year}). { link html%{“{← go p.title}”} }. <em>{← go p.journal}{journalDot}</em> <strong>{← go p.volume}</strong>{numberHtml}{pagesHtml}.}
  | .thesis p =>
...
where
  authorHtml p := open Html in do
    if p.authors.size = 0 then
      ...
      (· ++ html%{{.text " "}<em>et al.</em>}) <$> go (Bibliography.lastName p.authors[0])
