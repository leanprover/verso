  ...
  | {title, shortTitle := _, path, id, sectionNum, children} =>
    if depth = some 0 then .empty
      ...
      let sectionNum :=
        ...
        | some ns => html%{<span class="number">{sectionNumberString ns}</span>{.text " "}}
      html%{
        <li>
          <a href={page}>{sectionNum}{title}</a>{if children.isEmpty || depth == some 1 then .empty
    ...
    <html>
      ...
      <body>
        ...
        <label for="toggle-toc" id="toggle-toc-click">
          <span class="line line1"/><span class="line line2"/><span class="line line3"/>
        </label>
        <div class="with-toc">
          ...
          <nav id="toc">
            ...
            <div class="first">
              <a href={if let some dest := logoLink then dest else "/"} class="toc-title"><h1>{bookTitle}</h1></a>{toc.localHtml path localItems}
            </div>
            <div class="last">
