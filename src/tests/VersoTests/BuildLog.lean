...
def rangeFormat : Test := do
  ...
  assertBEq "CrossLine.lean:20:4-21:7: msg" crossLine.format
  ...
  assertBEq "SameLine.lean:43:4-43:21: msg" sameLine.format
...
def fileLocationFormat : Test := do
  ...
  assertBEq (some "FileLoc.lean") (m.loc.map (·.file))
...
def exitCode : Test := do
  ...
  assertBEq 2 (← withErrors.errors).size
  assertBEq 1 (← withErrors.warnings).size
  assertBEq 1 (← withErrors.exitCode)
  let warningOnly ← Logger.new
  (reportWarning "just a warning" : BuildLogT IO Unit).run warningOnly
  assertBEq 0 (← warningOnly.exitCode)
...
def ambientStderr : Test := do
  ...
  assertBEq 1 (← logger.errors).size
  assertBEq 1 (← logger.warnings).size
