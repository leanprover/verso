...
def mapperToJs : Test := do
  ...
  assertTrue (omits rendered "searchPriority") "mapper output should not contain `searchPriority`"
...
def priorityMap : Test := do
  ...
  for omitted in ["no-priority", "explicit-neutral"] do
    assertTrue (omits rendered omitted) s!"priorityMapJson should omit the neutral doc {omitted}"
