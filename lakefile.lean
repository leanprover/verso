...
-/
private def sourceHasTests (lines : List String) : Bool :=
  lines.any fun line =>
...
private partial def leanFiles (dir : System.FilePath) : IO (Array System.FilePath) := do
  unless ← dir.pathExists do return #[]
  let mut out := #[]
  for entry in ← dir.readDir do
    if ← entry.path.isDir then
      out := out ++ (← leanFiles entry.path)
    else if entry.path.extension == some "lean" then
      out := out.push entry.path
  return out

/-- The module name of a `.lean` file relative to a source directory, if it lies within it. -/
private def moduleOfPath (srcDir path : System.FilePath) : Option Lean.Name := do
...
-/
private def warnUncoveredTestModules (ws : Lake.Workspace) : IO Unit := do
  ...
  for lib in ws.root.leanLibs do
    ...
    for path in ← leanFiles srcDir do
      let some mod := moduleOfPath srcDir path | continue
...
script run (args) do
  ...
  let modInfos ← runBuild do
    ...
    pure <| (Job.collectArray oleanJobs).map (sync := true) fun _ => infos
