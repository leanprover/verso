...
/-- Whether a source file introduces Errata tests, by an `@[test]` attribute, a `#test_msgs`
command, or a `#test_guard` command. This drives the glob-coverage check, which reads source files
before anything is built; test discovery itself reads the compiled modules. -/
private def errataSourceHasTests (lines : List String) : Bool :=
  lines.any fun line =>
    let t := line.trimAsciiStart.copy
    t.startsWith "@[test]" || t.startsWith "#test_msgs" || t.startsWith "#test_guard"
...
private def errataSplitArgs (args : List String) : Except String (List String × List String) :=
  ...
  | some opt =>
    .error s!"unexpected option '{opt}' among library names; pass runner options after \
