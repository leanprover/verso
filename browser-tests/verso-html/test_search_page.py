"""End-to-end tests for the full-page search results view, VersoHtml genre.

The VersoHtml genre produces a minimal HTML site from literate JSON input. Run this
suite with `--site-dir` pointing at a VersoHtml output directory, e.g.

    lake exe verso-html test-projects/literate-config/.lake/build/literate _out/verso-html
    uv run --project browser-tests --extra test pytest browser-tests/verso-html \
